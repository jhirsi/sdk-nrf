 ...
 */
struct dect_scan_beacon_params {
 ...
 */
struct dect_association_response_params {
 ...
 */
struct dect_association_result {
 ...
 */
struct dect_association_release_result {
	bool association_changed_received; /**< True if NET_EVENT_DECT_ASSOCIATION_CHANGED rcvd */
...
struct dect_activate_result {
	...
	bool functional_mode_set_called; /**< True if nrf_modem_dect_control_functional_mode_set
					   * was called
					   */
};
 ...
 */
struct dect_rssi_scan_result {
	bool rssi_scan_result_received; /**< True if NET_EVENT_DECT_RSSI_SCAN_RESULT was received */
	bool rssi_scan_done_received;    /**< True if NET_EVENT_DECT_RSSI_SCAN_DONE was received */
 ...
 * 2. Verifies that nrf_modem_dect_mac_rssi_scan() is called
 * 3. Waits for async RSSI scan result notification (rssi_scan_ntf callback)
 * 4. Verifies NET_EVENT_DECT_RSSI_SCAN_RESULT is received
 * 5. Waits for async RSSI scan completion (rssi_scan op callback)
 * 6. Verifies NET_EVENT_DECT_RSSI_SCAN_DONE is received with success status
 * 7. Returns RSSI scan results
 *
 ...
 * @param result Pointer to result structure to populate. Must not be NULL.
 *
 * @return 0 on success (RSSI scan request accepted), negative error code on failure
 */
