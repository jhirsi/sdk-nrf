...
You can use the Location tool for retrieving devices location with different location methods.
See :ref:`lib_location` library for information on the configuration of different location methods and services
but there are some available by default to ease trials.
...
* Retrieve location with Wi-Fi positioning (you need to have Wi-Fi build and chip) and if not found, use cellular positioning:
