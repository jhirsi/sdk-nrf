...
* PPP network interface is setup/setdown automatically when LTE connection is up/down. Set the PPP network interface up manually:
...
* Set the rts_cts HW flow control configuration for PPP UART (default: none):
...
After flashing, testing with the development kit and Linux can be done as follows for example:

1. Connect the development kit to the computer using a USB cable.
   The development kit is assigned a ttyACM device (Linux).

#. Create a serial connection to the development kit (/dev/ttyACM2) with a terminal (e.g. PuTTY).
...
#. Type ``ppp uartconf`` commands resulting for example following uart configuration:
...
#. In Linux terminal start the ppp connection by typing following:
...
#. In MoSh terminal observe that ppp connection is created:
...
#. Now you are ready to browse Internet in Linux by using setuped MoSh ppp dialup over LTE connection.
