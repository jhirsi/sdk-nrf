	...
	for (int i = 0; i < loc_conf->methods_count; i++) {
		...
		if (loc_conf->methods[i].method == LOCATION_METHOD_GNSS) {
			...
			if (json_add_num_cs(loc_method_conf_obj, "cell_timeout",
			    ...
			    json_add_num_cs(
				    loc_method_conf_obj, "cell_ncellmeas_at_search_type",
				    loc_conf->methods[i].cellular.ncellmeas_params.search_type -
					    1) ||
			    json_add_num_cs(
				    loc_method_conf_obj, "cell_ncellmeas_at_gci_count",
				    loc_conf->methods[i].cellular.ncellmeas_params.gci_count)) {
					       ...
					       char **json_str_out)
{
	...
	used_time_sec = details->used_time_sec;
