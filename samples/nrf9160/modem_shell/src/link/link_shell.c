...
static const char link_ncellmeas_usage_str[] =
	...
	"                      'default', 'ext_light', 'ext_comp', 'gci_1', 'gci_2' and 'gci_3'.\n"
	"Options for GCI search_types:\n"
	"   --gci_count, [int]  Result notification for GCI search types 3-5 include Cell ID, PLMN\n"
	"                       and TAC for up to <gci_count> cells, and optionally list of\n"
	"                       neighbour cell measurement results related to each cell.\n"
	...
	"\n"
	"                      GCI (Global Cell Id) search, option 1. Modem searches EARFCNs\n"
	"                      based on previous cell history.\n"
	"\n"
	"                      GCI search, option 2. Modem starts with the same search method\n"
	"                      than in search_type gci_1.\n"
	...
	"\n"
	"                      GCI search, option 3. Modem starts with the same search method\n"
	"                      than in search_type gci_1.\n"
	...
	link_shell_string_to_ncellmeas_search_type(const char *search_type_str)
{
	...
	} else if (strcmp(search_type_str, "gci_1") == 0) {
		search_type = LTE_LC_NEIGHBOR_SEARCH_TYPE_GCI_OPTION_1;
	} else if (strcmp(search_type_str, "gci_2") == 0) {
		search_type = LTE_LC_NEIGHBOR_SEARCH_TYPE_GCI_OPTION_2;
	} else if (strcmp(search_type_str, "gci_3") == 0) {
		search_type = LTE_LC_NEIGHBOR_SEARCH_TYPE_GCI_OPTION_3;
	}
