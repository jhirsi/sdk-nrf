...
static int util_get_pdn_conn_dyn_params(int cid, struct pdn_conn_dyn_params *ret_dyn_info)
{
	...
	char *next_param_str;
	char at_cmd_str[16];
	char at_cmd_response_str[1024];
	char dns_addr_str[256];
	...
	while (resp_continues) {
		...
		if (iterator == 1) {
			ret = at_params_int_get(&param_list,
						...
						&(ret_dyn_info->ipv6_mtu));
			if (ret) {
				/* Don't care if it fails: */
				ret = 0;
				ret_dyn_info->ipv6_mtu = 0;
			}
		} else {
			ret = at_params_int_get(&param_list,
						AT_CMD_PDP_CONTEXT_READ_DYN_INFO_MTU_INDEX,
						&(ret_dyn_info->ipv4_mtu));
			if (ret) {
				/* Don't care if it fails: */
				ret = 0;
				ret_dyn_info->ipv4_mtu = 0;
			}
		}
		if (resp_continues) {
...
NET_DEVICE_INIT(nrf91_non_offload, "nrf91_nrf_non_offloading_dev",
		nrf91_nrf_modem_lib_non_offload_init, NULL,
