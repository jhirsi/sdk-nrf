...
static void ppp_ctrl_close_sckts(void)
{
	...
	/* We have set SO_RCVTIMEO for workarounding problems that are seen
	 * if closing sckt from another thread while same sckt in nrf_recv(). Thus,
	 * give some time for closing zephyr sckt and time to get away from recv()
	 * in modem sckt.
	 */
	k_sleep(K_SECONDS(2));
