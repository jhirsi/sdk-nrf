...
enum nrf_cloud_ground_fix_type active_cell_pos_type = GROUND_FIX_TYPE_SINGLE_CELL;
...
static struct nrf_cloud_rest_ground_fix_request cell_pos_req = {
	.cell_info = &cell_info,
	.wifi_info = NULL
};
...
static void get_cell_info(void)
{
	...
	if (active_cell_pos_type == GROUND_FIX_TYPE_SINGLE_CELL) {
		...
		cell_info.current_cell.timing_advance	= NRF_CLOUD_GROUND_FIX_CELL_OMIT_TIME_ADV;
		cell_info.current_cell.rsrq		= NRF_CLOUD_GROUND_FIX_CELL_OMIT_RSRQ;
		cell_info.current_cell.earfcn		= NRF_CLOUD_GROUND_FIX_CELL_OMIT_EARFCN;
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (ready) {
		...
		if (active_cell_pos_type == GROUND_FIX_TYPE_SINGLE_CELL) {
			active_cell_pos_type = GROUND_FIX_TYPE_MULTI_CELL;
		} else {
			active_cell_pos_type = GROUND_FIX_TYPE_SINGLE_CELL;
		}
...
void main(void)
{
	...
	while (1) {
		struct nrf_cloud_ground_fix_result cell_pos_result;
		...
		/* Perform REST call */
		err = nrf_cloud_rest_ground_fix_get(&rest_ctx, &cell_pos_req, &cell_pos_result);
		...
		LOG_INF("Cellular location request fulfilled with %s",
			cell_pos_result.type == GROUND_FIX_TYPE_SINGLE_CELL ? "single-cell" :
			cell_pos_result.type == GROUND_FIX_TYPE_MULTI_CELL ?  "multi-cell" :
									      "unknown");
