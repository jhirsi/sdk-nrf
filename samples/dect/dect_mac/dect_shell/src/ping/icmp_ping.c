...
static uint32_t send_ping_wait_reply(struct icmp_ping_shell_cmd_argv *ping_args)
{
	...
	if (si->ai_family == AF_INET) {
		...
		struct sockaddr_in *sa = (struct sockaddr_in *)ping_args->src->ai_addr;
		...
		struct sockaddr_in6 *sa = (struct sockaddr_in6 *)ping_args->src->ai_addr;
	...
	fd = socket(si->ai_family, SOCK_RAW, htons(IPPROTO_IP));
	if (fd < 0) {
	...
	if (setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, (struct timeval *)&tv, sizeof(struct timeval)) <
	    0) {
		desh_warn("Unable to set socket SO_SNDTIMEO, continue");
	}
	...
	if (setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, (struct timeval *)&tv, sizeof(struct timeval)) <
	    0) {
		desh_warn("Unable to set socket SO_RCVTIMEO, continue");
	}
#endif
#endif
...
char *net_utils_sckt_addr_ntop(const struct sockaddr *addr)
{
	...
	if (addr->sa_family == AF_INET6) {
		return inet_ntop(AF_INET6, &net_sin6(addr)->sin6_addr, buf, sizeof(buf));
	}

	if (addr->sa_family == AF_INET) {
		return inet_ntop(AF_INET, &net_sin(addr)->sin_addr, buf, sizeof(buf));
	}
