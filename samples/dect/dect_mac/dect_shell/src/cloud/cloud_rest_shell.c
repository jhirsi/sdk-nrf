...
static int cmd_cloud_rest_do_jitp(void)
{
	...
	if (ret == 0) {
		...
		if (ret) {
			desh_warn("Failed to get device ID, error: %d", ret);
			return ret;
		}
		desh_print("Waiting 30s for cloud provisioning to complete...");
		k_sleep(K_SECONDS(30));

		desh_print("You can now add the device to nRF Cloud as %s", device_id);
		k_sleep(K_SECONDS(5));
	} else if (ret == 1) {
		desh_warn("Device has been already provisioned to nRF Cloud");
		ret = 0;
	} else {
		desh_error("JITP device provisioning failed: %d", ret);
	}
...
static int cmd_cloud_rest_jitp(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (err) {
		desh_error("Failed to perform JITP, %d", err);
		return err;
	}
	return 0;
}
...
{
	int err;
	struct nrf_cloud_modem_info modem_info = {.device = NRF_CLOUD_INFO_SET,
...
#define REST_RX_BUF_SZ 300 /* No payload in response, "just" headers */
	...
	if (err) {
		desh_error("Failed to get device ID, error: %d", err);
		return err;
	}

	err = nrf_cloud_rest_shadow_device_status_update(&rest_ctx, device_id, &device_status);
	if (err) {
		desh_error("REST: shadow update failed: %d", err);
		return err;
	}
	desh_print("Device status updated to cloud");
	return 0;
}

static int cloud_rest_init(void)
{
#if defined(CONFIG_NRF_CLOUD_PROVISION_CERTIFICATES)
	int err = nrf_cloud_credentials_provision();

	if (err) {
		printk("Failed to provision credentials, error: %d\n", err);
	}
#endif
...
SYS_INIT(cloud_rest_init, APPLICATION, CONFIG_KERNEL_INIT_PRIORITY_DEVICE);
...
SHELL_CMD_REGISTER(cloud_rest, &sub_cloud_rest, "Send nRF Cloud command over REST",
