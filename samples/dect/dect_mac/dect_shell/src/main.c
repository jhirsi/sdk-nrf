...
#if defined(CONFIG_DESH_FIDO_TRACE) && !defined(CONFIG_BOARD_THINGY91_NRF9160_NS)
#include <zephyr/init.h>
#include <nrf.h>
...
#if RM_JH /* alternative way */
	*(uint32_t *)0x40842754 = 0x302;
	*(uint32_t *)0x40842758 = 0x302;
	*(uint32_t *)0x4084275c = 0x302;
	*(uint32_t *)0x40842760 = 0x302;
	*(uint32_t *)0x40842764 = 0x302;
#endif
	return 0;
}

SYS_INIT(modem_trace_enable, PRE_KERNEL_1, 0);
#endif
