				    ...
				    void *cb_arg)
{
	...
	if (strcmp(key, AT_CMD_MODE_SETT_AUTOSTART) == 0) {
		...
		if (ret < 0) {
			desh_error("Failed to read sett_autostart_enabled, error: %d", ret);
			return ret;
		}
	}
	return 0;
...
int at_cmd_mode_sett_autostart_enabled(bool enabled)
{
	...
	if (err) {
		desh_error("sett_autostart_enabled: err %d from settings_save_one()", err);
		return err;
	}
...
int at_cmd_mode_sett_init(void)
{
	...
	if (ret) {
		desh_error("Failed to initialize settings subsystem, error: %d", ret);
		return ret;
	}

	ret = settings_register(&cfg);
	if (ret) {
		desh_error("Cannot register settings handler %d", ret);
		return ret;
	}

	ret = settings_load();
	if (ret) {
		desh_error("Cannot load settings %d", ret);
		return ret;
	}
	return ret;
}
