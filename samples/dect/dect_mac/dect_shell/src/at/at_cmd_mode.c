...
static enum line_termination term_mode = CONFIG_DESH_AT_CMD_MODE_TERMINATION;
...
AT_MONITOR(desh_at_cmd_mode_handler, ANY, at_cmd_mode_event_handler, PAUSED);
...
send:
	...
	k_work_submit(&at_cmd_mode_work.work);
}
...
static void at_cmd_mode_bypass_cb(const struct shell *sh, uint8_t *recv, size_t len)
{
	...
	if (escape) {
		/* Cannot use shell/desh_print in bybass callback */
		printk("===========================================================\n");
		printk("DECT NR+ SHELL AT command mode exited\n");

		shell_set_bypass(sh, NULL);
		at_cmd_mode_dont_print = false;
		at_monitor_pause(&desh_at_cmd_mode_handler);
...
int at_cmd_mode_start(const struct shell *sh)
{
	...
	printk("desh AT command mode started, press ctrl-x ctrl-q to escape\n");
	printk("===========================================================\n");
	at_cmd_mode_dont_print = true;
	at_cmd_mode_active = true;

	at_monitor_resume(&desh_at_cmd_mode_handler);
