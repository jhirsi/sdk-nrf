...
#if defined(CONFIG_DESH_CLOUD_MQTT)
/** Wrap all mdeosh prints into JSON and send to nRF Cloud over MQTT */
bool desh_print_cloud_echo;
#endif
...
bool create_timestamp_string(char *timestamp_buf, int timestamp_buf_len)
{
	...
	chars = snprintf(timestamp_buf, timestamp_buf_len, "[%04d-%02d-%02d %02d:%02d:%02d.%03d] ",
...
#if defined(CONFIG_DESH_CLOUD_MQTT)
	if (desh_print_cloud_echo) {
		struct nrf_cloud_sensor_data desh_cloud_print = {.type = NRF_CLOUD_DEVICE_INFO,
								 .data.ptr = desh_print_buf,
								 .data.len = strlen(desh_print_buf),
								 .ts_ms = NRF_CLOUD_NO_TIMESTAMP};

		nrf_cloud_sensor_data_stream(&desh_cloud_print);
	}
#endif
...
char *desh_print_version_str_get(void)
{
	static char version_str[256];

	(void)snprintf(version_str, sizeof(version_str), "v%s-%s-libmodem-%s", NCS_VERSION_STRING,
		       NCS_COMMIT_STRING, NRF_MODEM_BUILD_STRING);
...
int desh_get_device_uuid(char *buf, size_t buf_size)
{
	int ret;

	if (buf == NULL || buf_size < UUID_SIZE) {
		return -EINVAL;
	}

	ret = nrf_modem_at_scanf("AT%DEVICEUUID", "%%DEVICEUUID: %" STRINGIFY(UUID_SIZE) "[^\r\n]",
									      buf);
	return ret;
}
...
void desh_print_version_info(void)
{
	...
	desh_print("DESH version:       %s", desh_print_version_str_get());
	...
	if (ret == 0) {
		desh_print("HW version:         %s", info_str);
	} else {
		desh_print("Unable to obtain HW version, error: %d", ret);
	}

	ret = modem_info_string_get(MODEM_INFO_FW_VERSION, info_str, sizeof(info_str));
	if (ret >= 0) {
		desh_print("Modem FW version:   %s", info_str);
	} else {
		desh_print("Unable to obtain modem FW version, error: %d", ret);
	}

	ret = modem_info_get_fw_uuid(info_str, sizeof(info_str));
	if (ret == 0) {
		desh_print("Modem FW UUID:      %s", info_str);
	} else {
		desh_print("Unable to obtain modem FW UUID, error: %d", ret);
	}
#endif
	ret = desh_get_device_uuid(info_str, sizeof(info_str));
	if (ret > 0) {
		desh_print("Modem Device UUID:  %s", info_str);
	} else {
		desh_print("Unable to obtain modem Device UUID, error: %d", ret);
	}
}
