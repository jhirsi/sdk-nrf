...
struct tls_cred_buf {
	uint8_t buf[CONFIG_DESH_NATIVE_TLS_CREDENTIAL_BUFFER_SIZE]; /* Credentials. */
	sec_tag_t sec_tag;  /* sec_tag of the credentials in buf. */
	uint8_t type_flags; /* Credential types loaded in buf (1 << desh_cmng_type). */
};
static struct tls_cred_buf cred_buf[CONFIG_DESH_NATIVE_TLS_CREDENTIAL_BUFFER_COUNT] = {
	[0 ... CONFIG_DESH_NATIVE_TLS_CREDENTIAL_BUFFER_COUNT - 1] = {.sec_tag = -1}};
			       ...
			       void *param)
{
	...
	int err;
	char buf[sizeof("%CMNG: 2147483647,0\r\n") + 1];
...
static int get_credentials_len(sec_tag_t sec_tag, size_t *len)
{
	...
	const int err = snprintf(key, sizeof(key), "desh-keys/%d", sec_tag);
...
int desh_native_tls_store_credential(sec_tag_t sec_tag, uint16_t type, const void *data, size_t len)
{
...
#if RM_JH /* We are overwriting, why need to check what is there already? */
	...
	if ((cred_len + len + null_termination) > CONFIG_DESH_NATIVE_TLS_CREDENTIAL_BUFFER_SIZE) {
		printk("Increase CONFIG_DESH_NATIVE_TLS_CREDENTIAL_BUFFER_SIZE. cred_len %d, len "
		       "%d, null %d\n",
