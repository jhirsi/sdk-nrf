	...
	const char *at_response, struct lte_lc_cells_info *cells)
{
	...
	/*
	 * Response format for search types 0-2:
	 * High level:
	 * status[,
	 *	serving_cell[,neighbour_cell1,neighbour_cell2...][,TA_meas_time]]
	 *
	 * Detailed:
	 * %NCELLMEAS:
	 * status [,
	 ...
	 * High level:
	 * status[,
	 *	GCI_cell_info1,neighbour_count1[,neighbour_cell1_1,neighbour_cell1_2...],
	 *	GCI_cell_info2,neighbour_count2[,neighbour_cell2_1,neighbour_cell2_2...]...]
	 *
	 * Detailed:
	 * %NCELLMEAS: status
	 * [,<cell_id>,<plmn>,<tac>,<ta>,<ta_meas_time>,<earfcn>,<phys_cell_id>,<rsrp>,<rsrq>,
	 *		<meas_time>,<serving>,<neighbour_count>
	 *	[,<n_earfcn>1,<n_phys_cell_id>1,<n_rsrp>1,<n_rsrq>1,<time_diff>1]
	 *	[,<n_earfcn>2,<n_phys_cell_id>2,<n_rsrp>2,<n_rsrq>2,<time_diff>2]...],
	 *  <cell_id>,<plmn>,<tac>,<ta>,<ta_meas_time>,<earfcn>,<phys_cell_id>,<rsrp>,<rsrq>,
	...
	for (i = 0; curr_index < (param_count - (AT_NCELLMEAS_GCI_CELL_PARAMS_COUNT + 1)) &&
		...
		/* <cell_id>  */
		curr_index++;
		err = string_param_to_int(&resp_list, curr_index, &tmp, 16);
		if (err) {
		...
		curr_index++;
		err = at_params_string_get(&resp_list, curr_index,
				tmp_str, &len);
		if (err) {
		...
		err = string_param_to_int(&resp_list, curr_index, &tmp, 16);
		if (err) {
			LOG_ERR("Could not parse tac in i %d, error: %d", i, err);
			goto clean_exit;
		}
		//cells->gci_cells[i].tac = tmp;
		parsed_cell.tac = tmp;

		/* <ta> */
		curr_index++;
		err = at_params_int_get(&resp_list, curr_index,
					&tmp);
		if (err) {
			LOG_ERR("Could not parse ta, error: %d", err);
			goto clean_exit;
		}
		//cells->gci_cells[i].timing_advance = tmp;
		parsed_cell.timing_advance = tmp;
		...
		if (err) {
			LOG_ERR("Could not parse ta_meas_time, error: %d", err);
			goto clean_exit;
		}
		...
		/* <phys_cell_id> */
		curr_index++;
		err = at_params_short_get(&resp_list, curr_index,
					&parsed_cell.phys_cell_id);
		if (err) {
		...
		//cells->gci_cells[i].serving_cell = tmp_short;
		parsed_cell.serving_cell = tmp_short;
		...
		//cells->gci_cells[i].ncells_count = tmp_short;
		parsed_cell.ncells_count = tmp_short;

		if (parsed_cell.serving_cell) {
			int to_be_parsed_ncell_count = 0;

			/* This a current / serving cell.
			 * Handle neighbor cells only for the serving cell:
			 * in practice there could be only one serving cell.
			 */
			cells->current_cell = parsed_cell;
			if (parsed_cell.ncells_count != 0) {

				/* Allocate room for the parsed neighbor info. */
				if (parsed_cell.ncells_count >
					...
					to_be_parsed_ncell_count = parsed_cell.ncells_count;
				}
				ncells = k_calloc(
						to_be_parsed_ncell_count,
						sizeof(struct lte_lc_ncell));
				if (ncells == NULL) {
					LOG_ERR("Failed to allocate memory for the ncells"
						" (continue)");
					continue;
				}
			...
			for (j = 0; j < to_be_parsed_ncell_count; j++) {
				...
				/* <n_rsrp>j */
				curr_index++;
				err = at_params_int_get(&resp_list, curr_index, &tmp);
				if (err) {
					LOG_ERR("Could not parse n_rsrp, error: %d", err);
					goto clean_exit;
				}
				cells->neighbor_cells[j].rsrp = tmp;
				...
				cells->neighbor_cells[j].rsrq = tmp;
			...
			cells->gci_cells_count++; /* Only count for non-serving cell */
			k++;
		}
	}
