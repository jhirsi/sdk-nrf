...
/**@brief List element for event handler. */
 ...
 */
static struct notif_handler *notif_handler_list_find_node(struct notif_handler **prev_out,
	lte_lc_evt_handler_t handler)
{
...
int notif_handler_list_append_handler(lte_lc_evt_handler_t handler)
{
	...
	if (notif_handler_list_find_node(&to_ins, handler) != NULL) {
...
int notif_handler_list_remove_notif_handler(lte_lc_evt_handler_t handler)
{
	...
	curr = notif_handler_list_find_node(&prev, handler);
	if (curr == NULL) {
