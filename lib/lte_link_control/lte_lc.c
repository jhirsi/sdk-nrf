...
static void at_handler_ncellmeas_gci(const char *response)
{
	...
	err = parse_ncellmeas_gci(&ncellmeas_params, response, &evt.cells_info);
...
int lte_lc_neighbor_cell_measurement(struct lte_lc_ncellmeas_params params)
{
	 ...
	 * Starting from modem firmware v1.3.4 (TBD), plaa-plaa for CGI types.
	 */
	ncellmeas_params.gci_count = 1;
	if (params.search_type == LTE_LC_NEIGHBOR_SEARCH_TYPE_EXTENDED_LIGHT) {
		err = nrf_modem_at_printf("AT%%NCELLMEAS=1");
	} else if (params.search_type == LTE_LC_NEIGHBOR_SEARCH_TYPE_EXTENDED_COMPLETE) {
		err = nrf_modem_at_printf("AT%%NCELLMEAS=2");
	} else if (params.search_type == LTE_LC_NEIGHBOR_SEARCH_TYPE_GCI_OPTION_1) {
		ncellmeas_params.gci_count = params.gci_count;
		err = nrf_modem_at_printf("AT%%NCELLMEAS=3,%d", params.gci_count);
	} else if (params.search_type == LTE_LC_NEIGHBOR_SEARCH_TYPE_GCI_OPTION_2) {
		ncellmeas_params.gci_count = params.gci_count;
		err = nrf_modem_at_printf("AT%%NCELLMEAS=4,%d", params.gci_count);
	} else if (params.search_type == LTE_LC_NEIGHBOR_SEARCH_TYPE_GCI_OPTION_3) {
		ncellmeas_params.gci_count = params.gci_count;
		err = nrf_modem_at_printf("AT%%NCELLMEAS=5,%d", params.gci_count);
	} else {
		/* Defaulting to use LTE_LC_NEIGHBOR_SEARCH_TYPE_DEFAULT */
		err = nrf_modem_at_printf("AT%%NCELLMEAS");
	}
	ncellmeas_params.search_type = params.search_type;
