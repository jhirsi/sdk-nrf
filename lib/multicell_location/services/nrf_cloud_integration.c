...
#if defined(CONFIG_NRF_CLOUD_MQTT)
/* Verify that GROUND_FIX is enabled if MQTT is enabled */
BUILD_ASSERT(
	IS_ENABLED(CONFIG_NRF_CLOUD_GROUND_FIX),
	"If CONFIG_NRF_CLOUD_MQTT is enabled also CONFIG_NRF_CLOUD_GROUND_FIX must be enabled");
	...
	struct multicell_location *const location)
{
	...
	err = nrf_cloud_rest_ground_fix_get(&rest_ctx, &loc_req, &result);
	if (!err) {
