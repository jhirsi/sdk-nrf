...
int nrf_iperf3_mock_getsockdomain(struct iperf_test *test, int sock)
{
    ...
    if (nrf_iperf3_mock_getsockname(test, sock, (struct sockaddr *)&sa, &len) < 0) { /* platform getsockname() is not working */
        return -1;
    }
...
void warning(struct iperf_test *ipt, const char *str)
{
	...
	if (ipt != NULL && ipt->ram_file_len && ipt->ram_file_ptr != NULL) {
	...
	if (test->json_output) {
		cJSON_AddItemToObject(
			...
			iperf_json_printf(
				...
				test->protocol->name,
				(int64_t)test->num_streams,
				(int64_t)test->settings->blksize,
				(int64_t)test->omit, (int64_t)test->duration,
				(int64_t)test->settings->bytes,
				(int64_t)test->settings->blocks,
				test->reverse ? (int64_t)1 : (int64_t)0,
				(int64_t)test->settings->tos));
	} else {
		if (test->verbose) {
			if (test->settings->bytes)
				iperf_printf(test, test_start_bytes,
					     test->protocol->name,
					     test->num_streams,
				...
				iperf_printf(test, test_start_blocks,
					     test->protocol->name,
					     test->num_streams,
					     test->settings->blksize,
				...
				iperf_printf(test, test_start_time,
					     test->protocol->name,
					     test->num_streams,
					     test->settings->blksize,
					     test->omit, test->duration,
					     test->settings->tos);
		}
	}
}
...
void iperf_on_connect(struct iperf_test *test)
{
	...
	if (test->role == 'c') {
		if (test->json_output)
			cJSON_AddItemToObject(
				...
				iperf_json_printf("host: %s  port: %d",
						  test->server_hostname,
						  (int64_t)test->server_port));
		else {
			iperf_printf(test, report_connecting,
				     test->server_hostname, test->server_port);
			if (test->reverse)
				iperf_printf(test, report_reverse,
					     test->server_hostname);
		}
	} else {
		len = sizeof(sa);
		...
		if (nrf_iperf3_mock_getsockdomain(test, test->ctrl_sck) == AF_INET) {
			...
			inet_ntop(AF_INET6, &sa_in6P->sin6_addr, ipr,
				  sizeof(ipr));
	...
	if (test->json_output) {
		cJSON_AddStringToObject(test->json_start, "cookie",
					test->cookie);
...
int iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
	static struct option longopts[] = {
		...
		{ "repeating-payload", no_argument, NULL,
		...
		{ "udp-counters-64bit", no_argument, NULL,
		  OPT_UDP_COUNTERS_64BIT },
		{ "no-fq-socket-pacing", no_argument, NULL,
	...
	while ((flag = getopt(
		...
		case 'm':
			nrf_iperf3_usage();
			return -2; /* exit() not supported and help is special case */
		default:
			/* usage_long(stderr); */
			/* nrf_iperf3_usage(); */
			return -1; /* exit() not supported */
			/* exit(1); */
#else
	    case 'h':
		usage_long(stdout);
	...
	if (!rate_flag)
		test->settings->rate =
			test->protocol->id == Pudp ? UDP_RATE : 0;
...
int test_is_authorized(struct iperf_test *test)
{
	...
	if (test->settings->authtoken) {
		...
		time_t ts;
		int rc = decode_auth_setting(test->debug,
		...
		if (ret == 0) {
			iperf_printf(test, report_authentication_succeeded,
				     username, ts);
			free(username);
			free(password);
			return 0;
		} else {
			iperf_printf(test, report_authentication_failed,
				     username, ts);
			free(username);
			free(password);
			return -1;
...
static int send_parameters(struct iperf_test *test)
{
	...
	if (j == NULL) {
		...
		if (test->settings->bytes)
			cJSON_AddNumberToObject(j, "num",
						test->settings->bytes);
		if (test->settings->blocks)
			cJSON_AddNumberToObject(j, "blockcount",
						test->settings->blocks);
		if (test->settings->mss)
		...
		if (test->settings->blksize)
			cJSON_AddNumberToObject(j, "len",
						test->settings->blksize);
		if (test->settings->rate)
			cJSON_AddNumberToObject(j, "bandwidth",
						test->settings->rate);
		...
		if (test->settings->burst)
			cJSON_AddNumberToObject(j, "burst",
						test->settings->burst);
		if (test->settings->tos)
			cJSON_AddNumberToObject(j, "TOS", test->settings->tos);
		if (test->settings->flowlabel)
			cJSON_AddNumberToObject(j, "flowlabel",
						test->settings->flowlabel);
		if (test->title)
			cJSON_AddStringToObject(j, "title", test->title);
		if (test->extra_data)
			cJSON_AddStringToObject(j, "extra_data",
						test->extra_data);
		if (test->congestion)
			cJSON_AddStringToObject(j, "congestion",
						test->congestion);
...
static int get_parameters(struct iperf_test *test)
{
	...
	if (j == NULL) {
		...
		if ((j_p = cJSON_GetObjectItem(j, "udp_counters_64bit")) !=
		    NULL)
	...
	if (j == NULL) {
		...
		if (test->role == 's' && test->get_server_output) {
			if (test->json_output) {
				...
				TAILQ_FOREACH(t, &(test->server_output_list),
					      textlineentries)
				{
		...
		if (j_streams == NULL) {
			...
			SLIST_FOREACH(sp, &test->streams, streams)
			{
				...
				if (j_stream == NULL) {
					...
					retransmits =
						(sp->sender &&
						 test->sender_has_retransmits) ?
							sp->result
								->stream_retrans :
							-1;
					cJSON_AddNumberToObject(j_stream, "id",
								sp->id);
					cJSON_AddNumberToObject(
						j_stream, "bytes",
						bytes_transferred);
					cJSON_AddNumberToObject(j_stream,
								"retransmits",
								retransmits);
					cJSON_AddNumberToObject(
						j_stream, "jitter", sp->jitter);
					cJSON_AddNumberToObject(j_stream,
								"errors",
								sp->cnt_error);
					cJSON_AddNumberToObject(
					...
					iperf_time_diff(&sp->result->start_time,
							&sp->result->start_time,
							&temp_time);
					start_time =
						iperf_time_in_secs(&temp_time);
					iperf_time_diff(&sp->result->start_time,
							&sp->result->end_time,
							&temp_time);
					end_time =
...
static int get_results(struct iperf_test *test)
{
	...
	if (j == NULL) {
					...
					if (j_stream == NULL) {
						...
						j_retransmits =
							cJSON_GetObjectItem(
								j_stream,
								"retransmits");
						j_jitter = cJSON_GetObjectItem(
							j_stream, "jitter");
						j_errors = cJSON_GetObjectItem(
							j_stream, "errors");
						j_packets = cJSON_GetObjectItem(
							j_stream, "packets");
						j_start_time =
							cJSON_GetObjectItem(
								j_stream,
		 ...
		 */
				if (test->role == 'c' &&
					...
					if (j_server_output != NULL) {
						...
						j_server_output = cJSON_GetObjectItem(
							j,
							"server_output_text");
						if (j_server_output != NULL) {
							test->server_output_text = strdup(
								j_server_output
									->valuestring);
						}
					}
		...
		j_remote_congestion_used =
			cJSON_GetObjectItem(j, "congestion_used");
...
void connect_msg(struct iperf_stream *sp)
{
	...
	if (nrf_iperf3_mock_getsockdomain(sp->test, sp->socket) == AF_INET) {
		...
		inet_ntop(AF_INET,
			  (void *)&((struct sockaddr_in *)&sp->remote_addr)
				  ->sin_addr,
			  ipr, sizeof(ipr));
		mapped_v4_to_regular_v4(ipr);
		lport = ntohs(
			((struct sockaddr_in *)&sp->local_addr)->sin_port);
		rport = ntohs(
			((struct sockaddr_in *)&sp->remote_addr)->sin_port);
	} else {
		inet_ntop(AF_INET6,
			  (void *)&((struct sockaddr_in6 *)&sp->local_addr)
				  ->sin6_addr,
			  ipl, sizeof(ipl));
		mapped_v4_to_regular_v4(ipl);
		inet_ntop(AF_INET6,
			  (void *)&((struct sockaddr_in6 *)&sp->remote_addr)
				  ->sin6_addr,
			  ipr, sizeof(ipr));
		mapped_v4_to_regular_v4(ipr);
		lport = ntohs(
			((struct sockaddr_in6 *)&sp->local_addr)->sin6_port);
		rport = ntohs(
			((struct sockaddr_in6 *)&sp->remote_addr)->sin6_port);
	}
	...
	if (sp->test->json_output)
		cJSON_AddItemToArray(
			...
			iperf_json_printf(
				...
				(int64_t)sp->socket, ipl, (int64_t)lport, ipr,
				(int64_t)rport));
	else
...
#if defined(CONFIG_NRF_IPERF3_INTEGRATION)
	testp->settings->domain = AF_INET; /* AF_UNSPEC not supported by libmodem/modem */
#else
...
void iperf_reset_stats(struct iperf_test *test)
{
	...
	SLIST_FOREACH(sp, &test->streams, streams)
	{
		...
		rp->bytes_received = 0;
		rp->bytes_sent_this_interval =
			rp->bytes_received_this_interval = 0;
		if (test->sender_has_retransmits == 1) {
			struct iperf_interval_results
				ir; /* temporary results structure */
			save_tcpinfo(sp, &ir);
			rp->stream_prev_total_retrans =
				get_total_retransmits(&ir);
		}
		rp->stream_retrans = 0;
		rp->start_time = now;
	}
...
void iperf_stats_callback(struct iperf_test *test)
{
	...
	SLIST_FOREACH(sp, &test->streams, streams)
	{
		...
		temp.bytes_transferred =
			sp->sender ? rp->bytes_sent_this_interval :
		...
		memcpy(&temp.interval_end_time, &rp->end_time,
		       sizeof(struct iperf_time));
		iperf_time_diff(&temp.interval_start_time,
				&temp.interval_end_time, &temp_time);
		...
		if (test->protocol->id == Ptcp) {
			...
			if (irp == NULL) {
				...
				temp.interval_outoforder_packets =
					sp->outoforder_packets -
					irp->outoforder_packets;
				temp.interval_cnt_error =
					sp->cnt_error - irp->cnt_error;
			}
...
static void iperf_print_intermediate(struct iperf_test *test)
{
	...
	if (test->json_output) {
		...
		if (json_interval_streams == NULL)
			return;
		cJSON_AddItemToObject(json_interval, "streams",
...
static void iperf_print_results(struct iperf_test *test)
{
	...
	if (test->json_output) {
		...
		if (json_summary_streams == NULL)
			return;
		cJSON_AddItemToObject(test->json_end, "streams",
	...
	for (current_mode = lower_mode; current_mode <= upper_mode;
		...
		double start_time, end_time = 0.0, avg_jitter = 0.0,
				   lost_percent = 0.0;
			...
			SLIST_FOREACH(sp, &test->streams, streams)
			{
				if (sp->sender == stream_must_be_sender) {
					...
					unit_snprintf(ubuf, UNIT_LEN,
						      (double)bytes_sent, 'A');
					if (sender_time > 0.0) {
						bandwidth = (double)bytes_sent /
							    (double)sender_time;
					} else {
						bandwidth = 0.0;
					}
					unit_snprintf(
						nbuf, UNIT_LEN, bandwidth,
						test->settings->unit_format);
					if (test->protocol->id == Ptcp ||
					    test->protocol->id == Psctp) {
						if (test->sender_has_retransmits) {
					...
					if (sp->diskfile_fd >= 0) {
#if !defined(CONFIG_NRF_IPERF3_INTEGRATION)
/* Not supported and also: fstat might requires _times from newlib -> flagged out */
						if (fstat(sp->diskfile_fd,
							  &sb) == 0) {
							/* In the odd case that it's a zero-sized file, say it was all transferred. */
							int percent_sent = 100,
							    percent_received =
								    100;
							if (sb.st_size > 0) {
								percent_sent =
									(int)(((double)bytes_sent /
									       (double)sb
										       .st_size) *
									      100.0);
								percent_received =
		...
		if (test->num_streams > 1 || test->json_output) {
			...
			if (test->protocol->id == Ptcp ||
				...
				if (test->json_output)
					...
					iperf_printf(test, report_sum_bw_format,
						     mbuf, start_time,
				...
				if (total_packets > 0) {
					lost_percent = 100.0 * lost_packets /
						       total_packets;
				} else {
					lost_percent = 0.0;
				}
		...
		if (test->json_output && current_mode == upper_mode) {
			cJSON_AddItemToObject(
				...
				iperf_json_printf(
					...
					(double)test->cpu_util[0],
					(double)test->cpu_util[1],
					(double)test->cpu_util[2],
					(double)test->remote_cpu_util[0],
					(double)test->remote_cpu_util[1],
					(double)test->remote_cpu_util[2]));
			if (test->protocol->id == Ptcp) {
				char *snd_congestion = NULL,
				     *rcv_congestion = NULL;
				if (stream_must_be_sender) {
			...
			if (test->verbose) {
				...
				if (test->protocol->id == Ptcp) {
					...
					if (snd_congestion) {
						iperf_printf(
							test,
							"snd_tcp_congestion %s\n",
							snd_congestion);
					}
					if (rcv_congestion) {
						iperf_printf(
							test,
							"rcv_tcp_congestion %s\n",
							rcv_congestion);
					}
				}
			}
	...
	if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
		if (test->sender_has_retransmits == 1 && sp->sender) {
			...
			if (test->json_output)
				...
				unit_snprintf(cbuf, UNIT_LEN, irp->snd_cwnd,
					      'A');
				iperf_printf(
					test, report_bw_retrans_cwnd_format,
					sp->socket, mbuf, st, et, ubuf, nbuf,
					irp->interval_retrans, cbuf,
		...
		if (sp->sender) {
			if (test->json_output)
				...
				iperf_printf(
					test, report_bw_udp_sender_format,
					sp->socket, mbuf, st, et, ubuf, nbuf,
					zbuf, irp->interval_packet_count,
					irp->omitted ? report_omitted : "");
		} else {
			...
			if (test->json_output)
				...
				iperf_printf(test, report_bw_udp_format,
					     sp->socket, mbuf, st, et, ubuf,
					     nbuf, irp->jitter * 1000.0,
				      ...
				      int sender)
{
	...
	sp->result = (struct iperf_stream_result *)malloc(
...
static int diskfile_send(struct iperf_stream *sp)
{
	...
	if (sp->diskfile_left < sp->test->settings->blksize &&
	    !sp->test->done) {
	...
	if (sp->diskfile_left &&
	    sp->diskfile_left < sp->test->settings->blksize) {
		memcpy(sp->buffer,
		       sp->buffer + (sp->test->settings->blksize -
				     sp->diskfile_left),
		       sp->diskfile_left);
		if (sp->test->debug)
			printf("Shifting %d bytes by %d\n", sp->diskfile_left,
			       (sp->test->settings->blksize -
				sp->diskfile_left));
	}
	return r;
}
...
void iperf_got_sigend(struct iperf_test *test)
{
	...
	if (test->ctrl_sck >= 0) {
		test->state = (test->role == 'c') ? CLIENT_TERMINATE :
						    SERVER_TERMINATE;
...
#if !defined(CONFIG_NRF_IPERF3_INTEGRATION) /* No file support */
...
int iperf_json_start(struct iperf_test *test)
{
	...
	if (test->json_connected == NULL)
		return -1;
	cJSON_AddItemToObject(test->json_start, "connected",
			      test->json_connected);
	test->json_intervals = cJSON_CreateArray();
	if (test->json_intervals == NULL)
		return -1;
	cJSON_AddItemToObject(test->json_top, "intervals",
			      test->json_intervals);
...
int iperf_json_finish(struct iperf_test *test)
{
	...
	test->json_top = test->json_start = test->json_connected =
		test->json_intervals = test->json_server_output =
			test->json_end = NULL;
	return 0;
}
	...
	if (test->role == 'c') {
#if defined(CONFIG_NRF_IPERF3_INTEGRATION)
		/* Write to RAM "file" if requested: */
		if (test->ram_file_len && test->ram_file_ptr != NULL) {
		...
		if (test->role == 's' &&
			...
			struct iperf_textline *l =
				(struct iperf_textline *)malloc(
					sizeof(struct iperf_textline));
			l->line = strdup(linebuffer);
			TAILQ_INSERT_TAIL(&(test->server_output_list), l,
					  textlineentries);
		}
	}
	return r;
