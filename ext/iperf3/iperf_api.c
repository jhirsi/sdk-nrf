    ...
    while ((flag = getopt_long(argc, argv, "p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
		...
		case NRF_OPT_PDN_ID: {
				...
				if (endptr == optarg || tmp < 0) {
					iperf_printf(test, "not valid PDN ID %s\n", optarg);
					return -1;
				}

				test->pdn_id_str = strdup(optarg);
				if (test->pdn_id_str == NULL) {
					iperf_printf(test, "strdup failed for setting the PDN ID %s\n", optarg);
					i_errno = IENOMEMORY;
					return -1;
				}
			}
    ...
    */
    if (test->settings->bytes == 0 &&
        ...
        if( stat(test->diskfile_name, &st) == 0 ){
            ...
            if (test->debug)
                iperf_printf(test, "End condition set to file-size: %"PRIu64" bytes\n", test->settings->bytes);
        }
    ...
    */
    ...
    if (!test->curr_mdm_traces) {
        ...
        if (nrf_modem_at_printf(lightweight_mdm_trace) != 0) {
        	iperf_printf(test, "error when setting lightweight modem traces\n");
        }
        else {
        	iperf_printf(test, "note: custom traces \"%s\" was set for testing\n",
        		lightweight_mdm_trace);
        	iperf_printf(test, "note: use --curr-mdm-traces hook for the current ones\n\n");
        }
    }
#endif
#endif
...
get_results(struct iperf_test *test)
{
	...
	i_errno = IERECVRESULTS;
        r = -1;
	    if (test->debug) {
                iperf_printf(test, "IERECVRESULTS error\n");
	    }        
    } else {
...
iperf_free_test(struct iperf_test *test)
{
  ...
  if (!test->curr_mdm_traces) {
    ...
    if (nrf_modem_at_printf(default_mdm_trace) != 0) {
      iperf_printf(test, "error when setting default modem traces \"%s\"\n", default_mdm_trace);
    }
    else {
      iperf_printf(test, "note: default traces \"%s\" was set\n", default_mdm_trace);
    }
  }
#endif
#endif
...
iperf_print_intermediate(struct iperf_test *test)
{
	...
	if (irp) {
	    ...
	    if (test->debug) {
#if defined(CONFIG_NRF_IPERF3_INTEGRATION)
		iperf_printf(test, "interval_len %f bytes_transferred %d\n", interval_len, (uint32_t)irp->bytes_transferred);
#else
		iperf_printf(test, "interval_len %f bytes_transferred %" PRIu64 "\n", interval_len, irp->bytes_transferred);
#endif
	    }
...
#if defined(CONFIG_NRF_IPERF3_INTEGRATION)
        iperf_printf(sp->test, "getsockname() failed, continue\n");
#else
...
#if defined(CONFIG_NRF_IPERF3_INTEGRATION)
        iperf_printf(sp->test, "getpeername() failed, continue\n");
#else
        i_errno = IEINITSTREAM;
        return -1;
#endif
...
diskfile_send(struct iperf_stream *sp)
{
    ...
    if (sp->diskfile_left < sp->test->settings->blksize && !sp->test->done) {
    	...
    	rtot += r;
    	if (sp->test->debug) {
    	    iperf_printf(sp->test, "read %d bytes from file, %d total\n", r, rtot);
    	}
        ...
        if (buffer_left != sp->test->settings->blksize) {
            if (sp->test->debug)
                iperf_printf(sp->test, "possible eof\n");
    ...
    if( sp->test->done || buffer_left == 0 ){
        if (sp->test->debug)
              iperf_printf(sp->test, "already done\n");
        sp->test->done = 1;
        return 0;
    }
	...
	memcpy(sp->buffer,
	       ...
	       sp->diskfile_left);
	if (sp->test->debug)
	    iperf_printf(sp->test, "Shifting %d bytes by %d\n", sp->diskfile_left, (sp->test->settings->blksize - sp->diskfile_left));
    }
    return r;
}
