...
#define NRF_CLOUD_APP_ID_MAX_LEN     64
...
/**
 * @brief Application ID specific MQTT rx data handler function type.
 *
 * @param rx_buf Received MQTT data that is having the set appId.
 * 
 * @retval 0 If rx data was consumed by the handler.
 *           Otherwise, a (negative) error code is returned. * 
 */

typedef int (*nrf_cloud_app_id_specific_rx_data_handler_t)(const char *const rx_buf);
...
/**
 * @brief Set appId specific MQTT RX data handler.
 *
 * @param[in] rx_data_handler Callback function for handling received MQTT data that is
 *            is having app_id_str as a value of appId json key.
 *            If NULL, JSON result will be sent to the cloud event handler as
 *            an NRF_CLOUD_EVT_RX_DATA event.
 *
 * @param[in] app_id_str NULL terminated appId string
 *
 * @retval 0 If successful.
 *           Otherwise, a (negative) error code is returned.
 */
int nrf_cloud_app_id_specific_rx_data_handler_set(
	nrf_cloud_app_id_specific_rx_data_handler_t rx_data_handler,
