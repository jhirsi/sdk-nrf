...
static enum nrf_cloud_ground_fix_type cell_pos_type;
...
static void ncell_meas_mon(const char *notify)
{
	...
	/* omit timing_advance */
	cell_data.current_cell.timing_advance = NRF_CLOUD_GROUND_FIX_CELL_OMIT_TIME_ADV;
...
static void cell_pos_req_wk(struct k_work *work)
{
	...
	if (cell_pos_type == GROUND_FIX_TYPE_SINGLE_CELL) {
		struct lte_lc_cells_info scell_inf = {.ncells_count = 0};

		/* Obtain the single cell info from the modem */
		err = nrf_cloud_ground_fix_scell_data_get(&scell_inf.current_cell);
		/* Request ground fix location using only the single cell info */
		err = nrf_cloud_ground_fix_request(&scell_inf, NULL, true, NULL);
		if (err) {
		...
		if (ncell_meas_status == 0 && cell_data.current_cell.id != 0) {
			err = nrf_cloud_ground_fix_request(&cell_data, NULL, true, NULL);
			if (err) {
...
static void on_cloud_evt_cell_pos_data_received(const struct nrf_cloud_data *const data)
{
	...
	err = nrf_cloud_ground_fix_process(data->ptr, &result);
	if (err == 0) {
		if (ttft_start != 0) {
...
static void cloud_event_handler(const struct nrf_cloud_evt *evt)
{
	...
	case NRF_CLOUD_EVT_RX_DATA_GROUND_FIX:
		LOG_INF("NRF_CLOUD_EVT_RX_DATA_GROUND_FIX");
	...
	case AT_CMD_TYPE_SET_COMMAND:
		...
		if ((op == CELLPOS_START_SCELL || op == CELLPOS_START_MCELL) &&
			...
			if (op == CELLPOS_START_SCELL) {
				cell_pos_type = GROUND_FIX_TYPE_SINGLE_CELL;
			} else {
				cell_pos_type = GROUND_FIX_TYPE_MULTI_CELL;
			}
			ttft_start = k_uptime_get();
