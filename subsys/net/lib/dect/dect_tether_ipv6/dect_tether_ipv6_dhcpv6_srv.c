...
static int64_t dhcp_last_maint_ms;
#endif
...
static struct net_if *host_eth_iface(void)
{
...
static bool host_eth_ready(struct net_if *eth)
{
		       ...
		       bool include_ia)
{
	...
	if (include_ia &&
		...
		size_t need = 4U + 12U + (have_ula ? 28U : 0U) + (have_gua ? 28U : 0U) +
			      ((!have_ula && !have_gua) ? (4U + 6U) : 0U);

		if (w + need > out_cap) {
			return -ENOSPC;
		}
		w += append_ia_na(out + w, po->iaid, ula, gua, have_ula, have_gua);
	}
...
#if IS_ENABLED(CONFIG_DECT_TETHER_IPV6_RA_TETHER_MAINT) && \
	CONFIG_DECT_TETHER_IPV6_DHCPV6_LEASE_REFRESH_MIN_MS > 0
static bool dhcp_send_proactive_reply(int fd, struct net_if *eth, struct net_if *dect)
{
	struct parse_out po;
	struct net_in6_addr ula, gua;
	bool have_ula, have_gua;
	...
	if (fd < 0 || !dhcp_lease.bound || dhcp_lease.client_duid_len == 0U) {
		return false;
	}

	(void)dect_tether_ipv6_dect_addrs_get(dect, &ula, &gua, &have_ula, &have_gua);
	if (!have_ula && dhcp_lease.have_ula) {
		ula = dhcp_lease.ula;
		have_ula = true;
	}
	if (!have_gua && dhcp_lease.have_gua) {
		gua = dhcp_lease.gua;
		have_gua = true;
	}
	if (!have_ula && !have_gua) {
		LOG_WRN("DHCPv6 proactive refresh: no ULA/GUA");
		return false;
	}

	memset(&po, 0, sizeof(po));
	memcpy(po.client_duid, dhcp_lease.client_duid, dhcp_lease.client_duid_len);
	po.client_duid_len = dhcp_lease.client_duid_len;
	po.iaid = dhcp_lease.iaid;
	po.have_clientid = true;
	po.have_iaid = true;

	fake_req[0] = DHCPV6_MSG_RENEW;
	fake_req[1] = dhcp_lease.tid[0];
	fake_req[2] = dhcp_lease.tid[1];
	fake_req[3] = dhcp_lease.tid[2];

	slen = build_reply(dhcp_msg_sbuf, sizeof(dhcp_msg_sbuf), DHCPV6_MSG_REPLY, fake_req,
			   sizeof(fake_req), &po, &ula, &gua, have_ula, have_gua, true);
	if (slen < 0) {
		LOG_WRN("DHCPv6 proactive refresh: build_reply failed %d", slen);
		return false;
	}

#if IS_ENABLED(CONFIG_DECT_TETHER_IPV6_FWD)
	dect_tether_ipv6_fwd_tether_update(eth, &dhcp_lease.cli, po.client_duid,
						po.client_duid_len, &ula, &gua, have_ula,
						have_gua);
#endif

	sent = zsock_sendto(fd, dhcp_msg_sbuf, (size_t)slen, 0,
			    (struct sockaddr *)&dhcp_lease.cli, sizeof(dhcp_lease.cli));
	if (sent != (ssize_t)slen) {
		LOG_WRN("DHCPv6 proactive refresh: sendto failed ret=%zd expect=%d errno=%d",
			sent, slen, errno);
		return false;
	}

	LOG_INF("DHCPv6 proactive Reply TX len=%d", slen);
	return true;
}

static bool dhcp_proactive_reply_try_send(void)
{
	int fd;
	struct net_if *eth;
	struct net_if *dect;

	if (!dhcp_lease.bound || atomic_get(&dhcp.stop)) {
		return false;
	}
	...
	return dhcp_send_proactive_reply(fd, eth, dect);
}

void dect_tether_ipv6_dhcpv6_srv_lease_refresh_request(void)
{
	int64_t now;
	int64_t min_ms = (int64_t)CONFIG_DECT_TETHER_IPV6_DHCPV6_LEASE_REFRESH_MIN_MS;

	if (min_ms > 0 && dhcp_last_maint_ms != 0) {
		now = k_uptime_get();
		if ((now - dhcp_last_maint_ms) < min_ms) {
			return;
		}
	}

	if (dhcp_proactive_reply_try_send()) {
		dhcp_last_maint_ms = k_uptime_get();
	}
}
#endif
			    ...
			    const struct net_in6_addr *gua, bool have_ula, bool have_gua)
{
	...
	if (dhcp_lease.bound && po->have_clientid && dhcp_lease.client_duid_len > 0U &&
	    dhcp_lease.client_duid_len == po->client_duid_len) {
		same_client = memcmp(po->client_duid, dhcp_lease.client_duid,
				     po->client_duid_len) == 0 &&
			      po->iaid == dhcp_lease.iaid;
	} else {
		same_client = false;
	}
	...
	} else if (dhcp_lease.bound && !same_client) {
...
#if IS_ENABLED(CONFIG_DECT_TETHER_IPV6_RA_TETHER_MAINT)
	dect_tether_ipv6_ra_tether_peer_note(&cli->sin6_addr, NULL, false);
#endif
}
...
{
	if (!dhcp_lease.bound || !dhcp_lease.have_gua || gua_out == NULL) {
		return false;
	}
...
static void dhcp_thread(void *p1, void *p2, void *p3)
{
	...
	while (!atomic_get(&dhcp.stop)) {
		struct net_if *eth = host_eth_iface();
		int fd = -1;
		 ...
		 */
		while (!atomic_get(&dhcp.stop) && !host_eth_ready(eth)) {
			k_sleep(K_MSEC(100));
			eth = host_eth_iface();
		}
		...
		while (!atomic_get(&dhcp.stop) && eth != NULL && net_if_is_up(eth)) {
...
#if IS_ENABLED(CONFIG_DECT_TETHER_IPV6_DHCPV6_RX_LOG)
			{
			...
			/* Renew/Rebind: if the DECT uplink is momentarily unavailable
			 * (brief re-association between the recvfrom and the addrs_get)
			 * fall back to the cached lease addresses.  Without this the
			 * Reply would carry NoAddrsAvail, which immediately deprecates
			 * the client's preferred addresses.  The check intentionally
			 * runs before the mtype switch because mtype is already known
			 * from the first byte of the received message.
			 */
			...
			if (mtype == DHCPV6_MSG_SOLICIT) {
				...
				/* RFC 3315 §18.2.4: Rebind contains no Server ID.
				...
				/* RFC 3315 §18.2.3: Confirm contains no Server ID.
				 * Reply with Status=Success if we have an active
				 * binding; otherwise stay silent (cannot determine
				 * if addresses are appropriate for the link).
		...
		slen = build_reply(dhcp_msg_sbuf, sizeof(dhcp_msg_sbuf), rtype,
				   dhcp_msg_rbuf, (size_t)n, &po, &ula, &gua, have_ula,
				   have_gua, include_ia);
			if (slen < 0) {
#if IS_ENABLED(CONFIG_DECT_TETHER_IPV6_DHCPV6_TRACE)
			...
			/* RFC 3315 §18.2.3: Reply to Confirm MUST carry Status=Success. */
...
#endif
			{
				...
				if (sent != (ssize_t)slen) {
					...
					LOG_INF("DHCPv6 Reply TX req=%u len=%d",
						(unsigned int)mtype, slen);
#if IS_ENABLED(CONFIG_DECT_TETHER_IPV6_DHCPV6_TRACE)
				} else {
...
#if IS_ENABLED(CONFIG_DECT_TETHER_IPV6_DHCPV6_TRACE)
		{
			struct net_if *cur = host_eth_iface();
			int if_up = -1;
			int carrier = -1;
...
void dect_tether_ipv6_dhcpv6_srv_uplink_lost(void)
{
	 ...
	 */
	k_work_schedule(&dhcp_revoke_dwork, K_SECONDS(10));
}
