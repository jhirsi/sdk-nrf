 ...
 * no extension headers) between the first Ethernet and first DECT interface.
 * Same framing assumptions as dect_bridge_routing_ll_mcast.c, without IID NAT.
 *
...
static struct net_if *bh6_mcast_iface_eth(void)
{
...
void dect_tether_ipv6_mdns_fwd_stop(void)
{
	...
	k_sleep(K_MSEC(BH6_MCAST_POLL_MS + 100));
...
int dect_tether_ipv6_mdns_fwd_start(void)
{
	...
	bh6_mdns_fwd.iface_eth = bh6_mcast_iface_eth();
	...
	k_thread_create(&bh6_mdns_fwd.thread, bh6_mdns_fwd_stack,
			K_THREAD_STACK_SIZEOF(bh6_mdns_fwd_stack), bh6_mdns_fwd_thread, NULL, NULL,
			NULL, BH6_MCAST_PRIO, 0, K_NO_WAIT);
	k_thread_name_set(&bh6_mdns_fwd.thread, "dect_bh6_mdns_fwd");
