...
/** RFC 4861: Managed address configuration (use DHCPv6 for addresses). */
#define RA_FLAG_MANAGED		0x80U

/** RFC 4861 §4.6.2: PIO flags — on-link (L) set, autonomous (A) clear. */
#define RA_PIO_FLAG_L		0x80U
...
static struct {
	...
	struct net_in6_addr tether_peer_ll;
	uint8_t tether_peer_mac[6];
	bool tether_peer_valid;
	bool tether_peer_have_mac;
#endif
	bool inited;
} bh_ra;
...
static void na_unsol_periodic_handler(struct k_work *work)
{
	...
	host_eth_send_unsolicited_na(host_eth_iface());
				...
				uint32_t pio_valid_lt, uint32_t pio_preferred_lt)
{
...

	{
		...
		/* PIO: L=1, A=0 for each prefix — on-link but not SLAAC. */
...
{
	struct net_if *host = host_eth_iface();
	const struct net_in6_addr *src_ll;
...
static void host_ra_emit(const struct net_in6_addr *ip_dst, const uint8_t *dst_mac_override)
{
	...
	IS_ENABLED(CONFIG_DECT_TETHER_IPV6_RA_TETHER_MAINT)
static void host_ra_emit_tether_ucast(const struct net_in6_addr *ll_dst, const uint8_t *mac,
				      bool mac_valid, bool include_gua)
{
	struct net_in6_addr gua;

	if (ll_dst != NULL) {
		if (mac_valid && mac != NULL) {
			host_ra_emit(ll_dst, mac);
		} else {
			host_ra_emit(ll_dst, NULL);
		}
	}
	if (include_gua && dect_tether_ipv6_dhcpv6_srv_tether_gua(&gua)) {
		if (mac_valid && mac != NULL) {
			host_ra_emit(&gua, mac);
		} else {
			host_ra_emit(&gua, NULL);
		}
	}
}
#endif /* tether maint */
...
{
	if (ll_addr == NULL || !net_ipv6_is_ll_addr(ll_addr)) {
		return;
	}

	bh_ra.tether_peer_ll = *ll_addr;
	bh_ra.tether_peer_valid = true;
	if (mac_valid && eth_mac != NULL) {
		memcpy(bh_ra.tether_peer_mac, eth_mac, sizeof(bh_ra.tether_peer_mac));
		bh_ra.tether_peer_have_mac = true;
	}
}

static void tether_maint_send(void)
{
	if (!bh_ra.tether_peer_valid || !dect_tether_ipv6_dhcpv6_srv_lease_bound()) {
		return;
	}
	if (!net_ipv6_is_ll_addr(&bh_ra.tether_peer_ll)) {
		return;
	}

	LOG_DBG("tether maint: unicast RA (+GUA) to host");
	if (bh_ra.tether_peer_have_mac) {
		host_ra_emit_tether_ucast(&bh_ra.tether_peer_ll, bh_ra.tether_peer_mac, true,
					  true);
	} else {
		host_ra_emit_tether_ucast(&bh_ra.tether_peer_ll, NULL, false, true);
	}
#if IS_ENABLED(CONFIG_DECT_TETHER_IPV6_RA_TETHER_MAINT) && \
	CONFIG_DECT_TETHER_IPV6_DHCPV6_LEASE_REFRESH_MIN_MS > 0
	dect_tether_ipv6_dhcpv6_srv_lease_refresh_request();
#endif
}
#endif
	...
	if (ip_hdr != NULL && ip_hdr->ipv6 != NULL) {
		...
		if (!net_ipv6_is_addr_unspecified(&rs_src) &&
			...
			if (lla != NULL && lla->len >= 6) {
				dect_tether_ipv6_ra_tether_peer_note(&rs_src, lla->addr,
									  true);
			} else {
				dect_tether_ipv6_ra_tether_peer_note(&rs_src, NULL, false);
			}
#endif
			if (lla != NULL && lla->len >= 6) {
