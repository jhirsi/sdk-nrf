...
bool dect_nrp_utils_net_linkaddr_set_from_long_rd_id(struct net_linkaddr *lladdr,
						     uint32_t long_rd_id)
{
	if (!lladdr) {
		return false;
...
uint32_t dect_nrp_utils_long_rd_id_from_ipv6_addr(struct in6_addr *dst_addr)
{
	uint32_t long_rd_id;
...
bool dect_nrp_utils_net_ipv6_addr_create_from_sink_and_long_rd_id(struct in6_addr prefix_64,
								  uint32_t sink_rd_id,
								  uint32_t own_rd_id,
