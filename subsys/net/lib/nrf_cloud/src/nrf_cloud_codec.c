...
{
	if (!topic) {
		return NRF_CLOUD_RCV_TOPIC_UNHANDLED;
	}
	...
	if (strstr(topic, TOPIC_VAL_RCV_AGPS)) {
		...
		return NRF_CLOUD_RCV_TOPIC_GROUND_FIX;
	...
	size_t inf_cnt, cJSON *const req_obj_out)
{
	...
	for (size_t i = 0; i < inf_cnt; ++i) {
		...
		/* optional */
		if ((cur->earfcn != NRF_CLOUD_GROUND_FIX_CELL_OMIT_EARFCN) &&
		    json_add_num_cs(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_EARFCN, cur->earfcn)) {
			goto cleanup;
		}

		if ((cur->rsrp != NRF_CLOUD_GROUND_FIX_CELL_OMIT_RSRP) &&
		    json_add_num_cs(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRP,
				    RSRP_IDX_TO_DBM(cur->rsrp))) {
			goto cleanup;
		}

		if ((cur->rsrq != NRF_CLOUD_GROUND_FIX_CELL_OMIT_RSRQ) &&
		    json_add_num_cs(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRQ,
				    RSRQ_IDX_TO_DB(cur->rsrq))) {
			goto cleanup;
		}

		if (cur->timing_advance != NRF_CLOUD_GROUND_FIX_CELL_OMIT_TIME_ADV) {
			uint16_t t_adv = cur->timing_advance;

			if (t_adv > NRF_CLOUD_GROUND_FIX_CELL_TIME_ADV_MAX) {
				t_adv = NRF_CLOUD_GROUND_FIX_CELL_TIME_ADV_MAX;
			}
		...
		for (uint8_t j = 0; nmr_array && (j < lte->ncells_count); ++j) {
			...
			/* optional */
			if ((ncell->rsrp != NRF_CLOUD_GROUND_FIX_CELL_OMIT_RSRP) &&
			    json_add_num_cs(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRP,
					    RSRP_IDX_TO_DBM(ncell->rsrp))) {
				goto cleanup;
			}
			if ((ncell->rsrq != NRF_CLOUD_GROUND_FIX_CELL_OMIT_RSRQ) &&
	...
	cJSON *const req_obj_out)
{
	...
	wifi_obj = cJSON_AddObjectToObjectCS(req_obj_out, NRF_CLOUD_GROUND_FIX_JSON_KEY_WIFI);
	ap_array = cJSON_AddArrayToObjectCS(wifi_obj, NRF_CLOUD_GROUND_FIX_JSON_KEY_APS);
	if (!ap_array) {
		goto cleanup;
	}

	for (uint8_t cnt = 0; cnt < wifi->cnt; ++cnt) {
		struct wifi_ap_info const *const ap = (wifi->ap_info + cnt);
		...
		/* required items */
		if (json_add_str_cs(ap_obj, NRF_CLOUD_GROUND_FIX_JSON_KEY_WIFI_MAC,
				    ap->mac_addr_str)) {
			goto cleanup;
		}
		...
		/* optional */
		if ((ap->ssid_str[0] != '\0') &&
		    json_add_str_cs(ap_obj, NRF_CLOUD_GROUND_FIX_JSON_KEY_WIFI_SSID,
				    ap->ssid_str)) {
			goto cleanup;
		}
		if ((ap->rssi != NRF_CLOUD_GROUND_FIX_WIFI_OMIT_RSSI) &&
		    json_add_num_cs(ap_obj, NRF_CLOUD_GROUND_FIX_JSON_KEY_WIFI_RSSI,
					ap->rssi)) {
			goto cleanup;
		}
		if ((ap->rssi != NRF_CLOUD_GROUND_FIX_WIFI_OMIT_CHAN) &&
		    json_add_num_cs(ap_obj, NRF_CLOUD_GROUND_FIX_JSON_KEY_WIFI_CH,
					ap->channel)) {
			goto cleanup;
		}
	}
...
cleanup:
	...
	cJSON_DeleteItemFromObject(req_obj_out, NRF_CLOUD_GROUND_FIX_JSON_KEY_WIFI);
...
static int nrf_cloud_parse_ground_fix_json(const cJSON *const gf_obj,
	struct nrf_cloud_ground_fix_result *const location_out)
{
	if (!gf_obj || !location_out) {
		return -EINVAL;
	}
	...
	lat = cJSON_GetObjectItem(gf_obj,
				  NRF_CLOUD_GROUND_FIX_JSON_KEY_LAT);
	lon = cJSON_GetObjectItem(gf_obj,
				  NRF_CLOUD_GROUND_FIX_JSON_KEY_LON);
	unc = cJSON_GetObjectItem(gf_obj,
				  NRF_CLOUD_GROUND_FIX_JSON_KEY_UNCERT);
	...
	location_out->type = GROUND_FIX_TYPE__INVALID;

	if (!get_string_from_obj(gf_obj, NRF_CLOUD_JSON_FULFILL_KEY, &type)) {
		if (!strcmp(type, NRF_CLOUD_GROUND_FIX_TYPE_VAL_MCELL)) {
			location_out->type = GROUND_FIX_TYPE_MULTI_CELL;
		} else if (!strcmp(type, NRF_CLOUD_GROUND_FIX_TYPE_VAL_SCELL)) {
			location_out->type = GROUND_FIX_TYPE_SINGLE_CELL;
		} else if (!strcmp(type, NRF_CLOUD_GROUND_FIX_TYPE_VAL_WIFI)) {
			location_out->type = GROUND_FIX_TYPE_WIFI;
		} else {
			LOG_WRN("Unhandled ground fix type: %s", type);
		}
	} else {
		LOG_WRN("Ground fix type not found in message");
	}
...
int nrf_cloud_parse_ground_fix_response(const char *const buf,
					struct nrf_cloud_ground_fix_result *result)
{
	int ret;
	cJSON *gf_obj;
	cJSON *data_obj;
	...
	gf_obj = cJSON_Parse(buf);
	if (!gf_obj) {
		LOG_DBG("No JSON found for ground fix");
		return 1;  /* 1: ground fix not found */
	}

	/* First, check to see if this is a REST payload, which is not wrapped in
	 * an nRF Cloud MQTT message
	 */
	ret = nrf_cloud_parse_ground_fix_json(gf_obj, result);
	if (ret == 0) {
		goto cleanup;
	}
	...
	if (!json_item_string_exists(gf_obj, NRF_CLOUD_JSON_MSG_TYPE_KEY,
				     NRF_CLOUD_JSON_MSG_TYPE_VAL_DATA) ||
	    !json_item_string_exists(gf_obj, NRF_CLOUD_JSON_APPID_KEY,
				     NRF_CLOUD_JSON_APPID_VAL_GROUND_FIX)) {
		/* Not a ground fix data message */
		goto cleanup;
	}

	/* MQTT payload format found, parse the data */
	data_obj = cJSON_GetObjectItem(gf_obj, NRF_CLOUD_JSON_DATA_KEY);
	if (data_obj) {
		ret = nrf_cloud_parse_ground_fix_json(data_obj, result);
		if (ret) {
			LOG_ERR("Failed to parse ground fix data");
		}
	...
	ret = get_error_code_value(gf_obj, &result->err);
	if (ret) {
		/* Indicate that an nRF Cloud error code was found */
		ret = -EFAULT;
	} else {
		/* No data or error was found */
		LOG_ERR("Expected data not found in ground fix message");
		ret = -EBADMSG;
	}
...
cleanup:
	...
	if (ret < 0) {
		...
		result->lon = 0.0;
		result->unc = 0;
		result->type = GROUND_FIX_TYPE__INVALID;
